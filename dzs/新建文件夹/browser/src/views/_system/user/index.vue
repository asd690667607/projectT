      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-tooltip content="编辑" placement="top">
           ...
           <el-tooltip content="锁定角色" placement="top">
            <el-button @click="handleLock(scope.$index,scope.row)" size="medium" type="info" icon="el-icon-goods" circle plain></el-button>
          </el-tooltip>
           <el-tooltip content="解锁角色" placement="top">
            <el-button @click="handleisLock(scope.$index,scope.row)" size="medium" type="info" icon="el-icon-sold-out" circle plain></el-button>
          </el-tooltip>
    ...
    methods: {
      ...
      handleLock(idx, row) {
        this.$confirm('您确定要锁定该用户？', '提示', confirm).then(() => {
          userApi.lockUser({uid: row.uid}).then(res => {
          // this.tableData.splice(idx, 1)

            --this.tablePage.totals
            this.dialogFormVisible = false
         ...
         //解除锁定
      handleisLock(idx, row) {
        this.$confirm('您确定要解锁该用户？', '提示', confirm).then(() => {
          userApi.islockUser({uid: row.uid}).then(res => {
          //  this.tableData.splice(idx, 1)
            --this.tablePage.totals
            this.dialogFormVisible = false
